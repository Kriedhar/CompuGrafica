...
class EnemigoEstatico(pygame.sprite.Sprite):
...
if __name__ == '__main__':
    ...
    #RECORTE BRUJA
    bruja=pygame.image.load("bruja2.png")
    infob=bruja.get_rect()

    an_img=infob[2]
    al_img=infob[3]
    print 'alto',al_img,'ancho',an_img

    al_corte = int(al_img)
    an_corte = int(an_img/17)
    print 'corteancho',an_corte


    spritebruja=[]
    for i in range(17):
            cuadrado=bruja.subsurface(i*an_corte,0*al_corte,an_corte,al_corte)
            spritebruja.append(cuadrado)
    ...
    while not fin:
        ...
        while menu:
            ...
            cantidad_estaticos = max_estaticos

            for t in todos:
            	todos.remove(t)
            for e in enemigos:
            	enemigos.remove(e)
            for est in enemigosestaticos:
                enemigosestaticos.remove(est)
            for i in range (cantidad_enemigos):
                e = Enemigo()
                e.rect.x = random.randrange(0,6020-e.rect.width)
                e.rect.y = random.randrange(0,ALTO-e.rect.height)
                enemigos.add(e)
                todos.add(e)
            for es in range(cantidad_estaticos):
                es = EnemigoEstatico(spritebruja)
                es.rect.x = random.randrange(0,6020-es.rect.width)
                es.rect.y = random.randrange(0,ALTO-es.rect.height)
                enemigosestaticos.add(es)
                todos.add(es)
            todos.add(p1)
            todos.add(p2)
        ...
        """for u in enemigosestaticos:
            ls_col = pygame.sprite.spritecollide(p1,estatico,False)
            for x in ls_col:
                if(abs(b.rect.bottom - jugador.rect.bottom)<=50)"""
